...
export const useAlerts = () => {
  ...
  const subscribe = useCallback(
    (handler: () => void) => {
      if (!conn) return () => {};
      conn.db.alerts.onInsert(handler);
      conn.db.alerts.onDelete(handler);
      conn.db.alerts.onUpdate(handler);
      return () => {
        conn.db.alerts.removeOnInsert(handler);
        conn.db.alerts.removeOnDelete(handler);
        conn.db.alerts.removeOnUpdate(handler);
      };
    },
    [conn],
  );
