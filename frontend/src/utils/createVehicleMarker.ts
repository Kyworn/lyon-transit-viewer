...
export const createVehicleMarker = (color: string, mode: string): string => {
  ...
  const markerColor = /^rgb(a?)\(/i.test(raw)
    ? raw
    : raw.startsWith('#')
      ? raw
      : /^[0-9a-f]{3}([0-9a-f]{3})?$/i.test(raw)
        ? `#${raw}`
        : '#FF0000';
    ...
    <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
      <defs>
        ...
        <filter id="neon-glow" x="-100%" y="-100%" width="300%" height="300%">
          ...
          <feFlood flood-color="${markerColor}" flood-opacity="0.8" result="glowColor"/>
      ...
      <g transform="translate(${center}, ${center})">
        ...
        <circle cx="0" cy="0" r="${coreRadius + 2}" fill="none" stroke="${markerColor}" stroke-width="2" filter="url(#neon-glow)" opacity="0.6">
        ...
        <circle cx="0" cy="0" r="${coreRadius}" fill="#000000" stroke="${markerColor}" stroke-width="${strokeWidth}"/>
